...
ax.plot(t_dense, bac_classical, 'b-', linewidth=2, label='Classical Model (Simulated)')
ax.plot(t_dense, bac_fractional, 'r--', linewidth=2, label='Fractional Model (Simulated)')

# Load and plot real TAC data
real_data_file = "d:\\Kentech\\2-1\\공업수학 1\\EM_Project\\bar+crawl+detecting+heavy+drinking\\data\\clean_tac\\BK7610_clean_TAC.csv"
participant_id = "BK7610"
try:
    tac_df = pd.read_csv(real_data_file)
    # Timestamps are in seconds, convert to hours from start
    tac_df['time_hours'] = (tac_df['timestamp'] - tac_df['timestamp'].iloc[0]) / 3600.0
    # TAC_Reading is in g/dL (which is g/100mL)
    ax.plot(tac_df['time_hours'], tac_df['TAC_Reading'], 'g.-', linewidth=1.5, markersize=4, label=f'Real TAC Data ({participant_id})')
...
ax.set_xlabel('Time (hours)')
ax.set_ylabel('BAC (g/100mL)') # Changed unit label
ax.set_title(f'Model Comparison with Real Data ({participant_id})')
ax.legend()
ax.grid(True, alpha=0.3)
